...
async def get_knowledge_base(
    ...
    # 查询指定ID的知识库
    result = await db.execute(
        select(KnowledgeBase).where(KnowledgeBase.id == knowledge_base_id).options(selectinload(KnowledgeBase.tags).selectinload(Tag.knowledge_bases), selectinload(KnowledgeBase.documents))  # 预加载tags，避免后续异步查询
    )
    # 返回查询结果，没有则返回None
...
async def get_knowledge_bases(
    ...
    # 查询知识库，支持分页
    result = await db.execute(
        select(KnowledgeBase).offset(skip).limit(limit).options(selectinload(KnowledgeBase.tags).selectinload(Tag.knowledge_bases), selectinload(KnowledgeBase.documents))
    )
    # 返回所有查询结果
...
async def update_knowledge_base(
    ...
    # 查询要更新的知识库
    result = await db.execute(
        select(KnowledgeBase).where(KnowledgeBase.id == knowledge_base_id).options(selectinload(KnowledgeBase.tags).selectinload(Tag.knowledge_bases), selectinload(KnowledgeBase.documents))
    )
